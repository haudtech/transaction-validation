...
| Payload validation (all fields required, amount > 0, valid currency) | Completed | Validation rules in `src/TransactionValidation.Core/Validation/PartnerTransactionValidator.cs` and ISO-4217 validation in `src/TransactionValidation.Core/Validation/PartnerTransactionRequestValidator.cs` | Unit tests under `tests/TransactionValidation.Tests/Unit/TransactionValidation.Core` and green unit suite result (37/37) |
